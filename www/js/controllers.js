...
//兑换
    .controller ('ExchangeCtrl', function ($location, $scope, $http, $state, $ionicLoading, $ionicPopup, $rootScope, locals, getUser) {
        ...
        getUser.getInfo (url + '/service/common/testToken?vid=' + vid + '&uid=' + uid)
        ...
        function disabledBtn3D () {
            if ($scope.totalMoney >= userInfo.data.user.voucher) {
                ...
                var alertPopup = $ionicPopup.alert ({
                    title: '<div class="popup-heads"><img src="./img/alert-success.png" alt="" width = "100%"></div>',
                    template: '<div class="alert-left">' +
                    '<p style="text-align: center">余额不足,请充值或删除多余的投注</p>' +
                    '</div>',
                    okText: '确 定',
                    okType: 'button-light'
                })
        ...
        function disabledBtn5D () {
            if ($scope.totalMoney >= userInfo.data.user.voucher) {
                ...
                var alertPopup = $ionicPopup.alert ({
                    title: '<div class="popup-heads"><img src="./img/alert-success.png" alt=""  width = "100%"></div>',
                    template: '<div class="alert-left">' +
                    '<p style="text-align: center">余额不足,无法操作,请充值</p>' +
                    '</div>',
                    okText: '确 定',
                    okType: 'button-light'
                })
...
//                        $scope.filterDataRed.push($scope.numDataRed[i]);
    ...
    .controller ('bettingDetailCtrl', function ($scope, $ionicPopup, $timeout, $state, $http, $stateParams, $ionicLoading) {
        ...
        $scope.manualAdd = function () {
            $state.go ('BigLotto-2',{},{reload:true});
